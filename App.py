...
up_file = st.file_uploader('Escolaha um arquivo CSV', type='csv')

def precessar_prever(df):
...
if up_file is not None:
    ...
    cluster = precessar_prever(df)
    df.insert(loc=0, column='grupos',value=cluster)

    st.write('Visualização dos 10 primeiros registros: ')
    st.write(df.head(10))

    csv = df.to_csv(index=False)
    st.download_button(label='baixar resultados completos', data=csv, file_name='Grupos_interresse.csv', mime='text/csv')
